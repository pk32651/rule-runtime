/**
 * AGNES Sub-Store Mihomo profile generator (V4 - Single-Device Douyin & Xiaohongshu Location Routing).
 *
 ...
 *
 * V4 Improvements (preserves all V3 optimizations):
 * 1. Single-device fine-grained location routing for Douyin & Xiaohongshu (10.10.10.231).
 * 2. Signaling / Account / Feed APIs routed to 【📍 社交定位】 (default: Hong Kong).
 * 3. Media CDN (douyinvod, xhscdn) remains 100% domestic DIRECT (fast, no proxy data burned).
 * 4. All other household devices completely isolated from this rule (normal domestic direct).
 * 5. Dedicated policy group 【📍 社交定位】 for instant toggle (DIRECT) or region switching.
 */
...
const AGNES_URLS = {
  ...
  agnes_ai: "https://raw.githubusercontent.com/pk32651/rule-runtime/refs/heads/main/AI.list?v=8",
  ...
  agnes_direct: "https://raw.githubusercontent.com/pk32651/rule-runtime/refs/heads/main/ADirect.list?v=4",
...
async function main(config = {}) {
  ...
  config.rules = [
    ...
    "IP-CIDR,192.168.0.0/16,➡️ 国内,no-resolve",
    "IP-CIDR,10.0.0.0/8,➡️ 国内,no-resolve",
    ...
    `AND,((SRC-IP-CIDR,${AGNES_TARGET_SPOOF_IP}/32),(DOMAIN-SUFFIX,douyinpic.com)),➡️ 国内`,
    ...
    `AND,((SRC-IP-CIDR,${AGNES_TARGET_SPOOF_IP}/32),(DOMAIN-SUFFIX,rednote.com)),📍 社交定位`,

    // 2. 自定义强制直连白名单（前置最高优先级保护，杜绝 snssdk.com 等国内业务被误杀）
